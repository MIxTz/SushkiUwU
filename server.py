...
class Database:
    ...
    def get_account_data(self, name: str) -> (list):
    ...
    with socket:
        while True:
           ...
           # if addr not in clients:
             ...
             #                )
             #   clients[addr] = [
             #       NetworkedClient(socket_map, addr, key),
             #       time() - IDLE_MAX_TIME + 5
             #   ]

             #   if data == b"\x05\x03\xff\x01":
             #       key = key.encode("ascii")
             #       socket_map.sendto(key, addr)
             #       print(">", key)
             #       key = None
           # else:
           #     try:
